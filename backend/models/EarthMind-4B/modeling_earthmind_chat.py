...
from .configuration_earthmind_chat import Sa2VAChatConfig
...
class Sa2VAChatModel(PreTrainedModel):
    ...
    def forward(self, data, data_samples=None, mode='loss', output_attentions=None):
    ...
    def generate_multi(
            self,
            pixel_values: Optional[torch.FloatTensor] = None,
            rgb_pixel_values: Optional[torch.FloatTensor] = None,
            input_ids: Optional[torch.FloatTensor] = None,
            attention_mask: Optional[torch.LongTensor] = None,
            visual_features: Optional[torch.FloatTensor] = None,
            generation_config: Optional[GenerationConfig] = None,
            ...
            if visual_features is not None:
                ...
                if type(pixel_values) is list or pixel_values.ndim == 5:
                    if type(pixel_values) is list:
                        ...
                        rgb_pixel_values = [
                            x.unsqueeze(0) if x.ndim == 3 else x for x in rgb_pixel_values
                        ]

                    # b*n, c, h, w
                    pixel_values = torch.cat(
                        [image.to(self.vision_model.dtype) for image in pixel_values], dim=0)
                    rgb_pixel_values = torch.cat(
                        [image.to(self.vision_model.dtype) for image in rgb_pixel_values], dim=0)
        ...
        with torch.no_grad():
            ...
            encode_outputs = self.language_model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                output_hidden_states=True,
                output_attentions=True,  # ✅ 添加这个！
                return_dict=True
            )
    ...
    def predict_forward_multi(
        ...
        if image is None and video is None and '<image>' not in past_text:
            ...
            input_dict['g_pixel_values'] = g_pixel_values
            input_dict['pixel_values'] = pixel_values
            input_dict['rgb_pixel_values'] = rgb_pixel_values
            ...
            ids = self.tokenizer.encode(input_text)
            ids = torch.tensor(ids).cuda().unsqueeze(0)
            image_token_mask = ids == self.img_context_token_id
            ...
            mm_inputs = {
                'pixel_values': input_dict['pixel_values'],
                'rgb_pixel_values': input_dict['rgb_pixel_values'],
                'input_ids': ids,
                'attention_mask': attention_mask,
                'position_ids': None,
                'past_key_values': None,
